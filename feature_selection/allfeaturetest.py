...
#For regression: f_regression RFE 递归消除特征
...
ridge = Ridge(alpha=7)
ridge.fit(X, Y)
ranks["Rid"] = rank_to_dict(np.abs(ridge.coef_), names)
lasso = Lasso(alpha=.05)
lasso.fit(X, Y)
ranks["Las"] = rank_to_dict(np.abs(lasso.coef_), names)
...
ranks["LINCor"] = rank_to_dict(f, names)
'''
mine = MINE()
mic_scores = []
